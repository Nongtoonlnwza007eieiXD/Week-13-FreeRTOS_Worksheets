...
// ✅ ทดลองที่ 2: Producer task - ให้ semaphore หลายครั้ง (Multiple Give)
void producer_task(void *pvParameters) {
    ...
    while (1) {
        ...
        event_counter++;
        ESP_LOGI(TAG, "🔥 Producer: Generating event #%d (Multiple Give)", event_counter);

        // ให้ semaphore 3 ครั้งติดกัน
        for (int i = 0; i < 3; i++) {
            if (xSemaphoreGive(xBinarySemaphore) == pdTRUE) {
                stats.signals_sent++;
                ESP_LOGI(TAG, "✓ Producer: Semaphore given (%d/3)", i + 1);
                gpio_set_level(LED_PRODUCER, 1);
                vTaskDelay(pdMS_TO_TICKS(100));
                gpio_set_level(LED_PRODUCER, 0);
            } else {
                ESP_LOGW(TAG, "✗ Producer: Semaphore already available (%d/3)", i + 1);
            }
            vTaskDelay(pdMS_TO_TICKS(100));
        }
    }
...
// ✅ ทดลองที่ 3: Consumer task - ตั้ง Timeout รอ 3 วินาที
void consumer_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (xSemaphoreTake(xBinarySemaphore, pdMS_TO_TICKS(3000)) == pdTRUE) {
            ...
            ESP_LOGW(TAG, "⏰ Consumer: Timeout after 3 seconds (no event)");
        }
    }
}
