...
#include <string.h>
#include <math.h>
#include <stdlib.h>              // <-- สำหรับ calloc
...
// ---------------- Forward Declarations (แก้ error undeclared/implicit) ----------------
...
static void feed_watchdog_callback(TimerHandle_t timer) {
    ...
    // จำลองปัญหา: หยุด feed ชั่วคราว ให้ watchdog timeout
    if (feed_count == 15) {
...
// ================= LED PATTERN SYSTEM =================
static inline void set_pattern_leds(bool led1, bool led2, bool led3) {
...
static float read_sensor_value(void) {
    ...
    float sensor_value = (voltage / 1000.0f) * 50.0f;
    sensor_value += (int32_t)(esp_random() % 100 - 50) / 100.0f;
...
static void sensor_timer_callback(TimerHandle_t timer) {
    ...
    // Software timer callback รันใน task ปกติ -> ใช้ API non-ISR
    ...
    if (sensor_data.value > 40.0f) {
        new_period = pdMS_TO_TICKS(500);
    } else if (sensor_data.value > 25.0f) {
...
static void init_hardware(void) {
    ...
    // ใช้ ATTEN รุ่นใหม่เพื่อลดคำเตือน deprecate
...
void app_main(void) {
    ...
    // เริ่มต้นด้วยแพทเทิร์น Slow Blink
