...
|-------|-------------|-----------------|--------------|----------------|------------|
| 1 (Normal) |25      | 24              |8             |3               | 96 %| 
| 2 (Multiple Give) |42 |29             |9             |4               | 69% |
| 3 (Short Timeout) |30 |18             |8             |2               | 60% |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
    จะมีเพียง “ครั้งแรกเท่านั้น” ที่มีผล เพราะ Binary Semaphore เก็บสถานะได้เพียง 2 แบบคือ
    0 (ยังไม่ได้ให้)
    1 (ให้แล้ว / พร้อมถูก take)
    ดังนั้นเมื่อมีการ xSemaphoreGive() หลายครั้งติดกันโดยที่ Consumer ยังไม่ได้ xSemaphoreTake() ค่าใน Semaphore จะค้างอยู่ที่ 1 เท่าเดิม ไม่สะสมจำนวน ทำให้ “สัญญาณเกิน” ถูกละทิ้งทันที

2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
    ต้องใช้ xSemaphoreGiveFromISR() เท่านั้น เพราะ ISR (Interrupt Service Routine) ทำงานในบริบทของ interrupt ซึ่งไม่สามารถเรียกฟังก์ชันที่อาจบล็อกหรือเปลี่ยน context ได้โดยตรง
    xSemaphoreGive() ใช้ใน Task context (โค้ดปกติ)
    xSemaphoreGiveFromISR() ใช้ใน ISR context เพื่อให้ FreeRTOS จัดการการสลับ context ได้อย่างปลอดภัยโดยไม่เกิด deadlock หรือ crash

3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
    Binary Semaphore เหมาะกับงานที่ต้องการ “สัญญาณเหตุการณ์” เช่น interrupt, timer, หรือ task trigger
    ส่วน Queue เหมาะกับงานที่ต้อง “ส่งข้อมูลจริง” (data payload) เช่น sensor readings หรือข้อความระหว่างโมดูล
